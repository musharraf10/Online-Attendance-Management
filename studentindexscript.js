...
const updateCalendar = (attendanceData) => {
  const calendarDays = daysTag.querySelectorAll('li');
  ...
  calendarDays.forEach((dayElement) => {
    ...
    const matchingAttendance = attendanceData.filter((record) => {
      const recordDate = new Date(record.stat_date);
      return (
    ...
    matchingAttendance.forEach((record) => {
      if (record.st_status === 'Present') {
        presentClasses += record.count;
      }
      totalClasses += record.count;
    });


    // Calculate the percentage based on the total classes (both present and absent)
    const percentage = (presentClasses / totalClasses) * 100;
    // console.log('presentClasses:', presentClasses);
...
// Call the initCalendar function to initialize the calendar
initCalendar();
prevNextIcon.forEach(icon => {
  icon.addEventListener("click", () => {
    ...
    fetch(`backend.php?year=${currYear}&month=${currMonth + 1}`)
      .then((response) => response.json())
      .then((data) => {
        updateCalendar(data);
      })
